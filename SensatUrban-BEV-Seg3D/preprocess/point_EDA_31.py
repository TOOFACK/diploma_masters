...
def _ensure_output_dirs(store: str) -> None:
    for sub in ("alt", "canny", "rgb", "cls", "vis"):
        os.makedirs(os.path.join(store, sub), exist_ok=True)


def _write_bev_tile(args) -> None:
    x_idx, y_idx, pts, grids_scale, grids_step, label_color_map, label_color_map_new, store, ply_name = args
    imgid = ply_name[:-4] + '_' + str(x_idx) + '_' + str(y_idx)
    alt, rgb, cla = hi.project_3d_bev_img3(pts.T, grids_scale, grids_step, label_color_map, imgid)
    ...
    for x in range(rgb.shape[0]):
        for y in range(rgb.shape[1]):
            vis[x, y, 2] = label_color_map_new[int(cla[x, y])][0]  # R
            vis[x, y, 1] = label_color_map_new[int(cla[x, y])][1]  # G
            vis[x, y, 0] = label_color_map_new[int(cla[x, y])][2]  # B
...
class SensatUrbanEDA(object):
    ...
    def load_points(self, ply_path: str, reformat: bool = False) -> np.array:
        ...
        if os.path.exists(ply_path):
            ...
            if reformat:
                ...
                g = _ply_data["green"]
                b = _ply_data["blue"]
                if "alpha" in _ply_data.dtype.names:
                    if r.max() == 0 and g.max() == 0 and b.max() == 0:
                        a = _ply_data["alpha"]
                        r = a.copy()
                        g = a.copy()
                        b = a.copy()
                if "class" in _ply_data.dtype.names:
                    c = _ply_data["class"]
                elif "cla" in _ply_data.dtype.names:
                    c = _ply_data["cla"]
                else:
                    c = np.zeros_like(x)
    ...
    def exp_gen_bev_projection(self, grids_data: Iterator[Tuple[int, int, np.array]], ply_name) -> list:
        ...
        _ensure_output_dirs(store)

        if getattr(self, "num_workers", 0):
            ctx = mp.get_context("fork")
            args_iter = (
                (x_idx, y_idx, pts, self.grids_scale, self.grids_step, self.label_color_map,
                 self.label_color_map_new, store, ply_name)
                for x_idx, y_idx, pts in grids_data
            )
            with ctx.Pool(processes=self.num_workers) as pool:
                for _ in pool.imap_unordered(_write_bev_tile, args_iter, chunksize=4):
                    pass
            return None
        ...
        for grid in grids_data:
            ...
            print('writing images to {}'.format(store))
            cv2.imwrite(store+'/alt/{}_alt50.png'.format(imgid), alt)
            cv2.imwrite(store+'/canny/{}_alt50.png'.format(imgid), alt_canny)
            cv2.imwrite(store+'/rgb/{}_rgb50.png'.format(imgid), rgb)
            cv2.imwrite(store+'/cls/{}_cls50.png'.format(imgid), cla)

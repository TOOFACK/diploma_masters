...
@torch.no_grad()
def predict_tiles_from_temp(model, metadata: dict, temp_dir: str, device: str = "cuda", max_tiles: Optional[int] = None) -> np.ndarray:
    """
...
def main():
    ...
    try:
        ...
        full_pred = predict_tiles_from_temp(model, metadata, temp_dir, device, max_tiles=args.debug_tiles)
        
        # Save results
        base_name = os.path.splitext(os.path.basename(args.input))[0]
        save_results(full_rgb, full_gt, full_pred, args.output_dir, base_name)
